...
[CustomEditor(typeof(ObjectPolling))]
...
public class ObjectPolling : MonoBehaviour
{
	...
	public static GameObject SpawnFromPool(string tag, Vector3 position, Transform parent, Quaternion rotation) =>
		inst._SpawnFromPool(tag, position,parent, rotation);

	public static T SpawnFromPool<T>(string tag, Vector3 position, Transform parent) where T : Component
	{
		GameObject obj = inst._SpawnFromPool(tag, position, parent, Quaternion.identity);
	...
	public static T SpawnFromPool<T>(string tag, Vector3 position, Transform parent, Quaternion rotation) where T : Component
	{
		GameObject obj = inst._SpawnFromPool(tag, position, parent,rotation);
		if (obj.TryGetComponent(out T component))
			return component;
		else
		{
	...
	GameObject _SpawnFromPool(string tag, Vector3 position, Transform parent, Quaternion rotation)
	{
		...
		if (poolQueue.Count <= 0)
		{
			...
			var obj = CreateNewObject(pool.tag, pool.prefab,pool.parent);
			ArrangePool(obj);
		}
	...
	void ArrangePool(GameObject obj)
	{
		...
		{
			if (i == obj.transform.parent.childCount - 1)
			{
				obj.transform.SetSiblingIndex(i);
				spawnObjects.Insert(i, obj);
				break;
			}
			else if (obj.transform.parent.GetChild(i).name == obj.name)
				isFind = true;
			else if (isFind)
			{
				obj.transform.SetSiblingIndex(i);
