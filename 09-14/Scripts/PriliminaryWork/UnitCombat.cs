...
public class UnitCombat : MonoBehaviour
{
    ...
    public class UnitCurrentInfo
    {
        ...
        public float currentUpgradeMulti;
    }
    ...
    private void InitializeStat()
    {
        ...
        currentInfo.currentReward = unitdata.reward;
        currentInfo.currentUpgradeCost = unitdata.upgradeCost;
        currentInfo.currentUpgradeMulti = unitdata.upgradeMulti;
    }
    ...
    private void DieProcess()
    {
        ...
        {
            if(dropItemMG)
                dropItemMG.ItemDrop(currentInfo.currentReward);
...
#region CurrentStat 반환
    ...
    {
        if (currentInfo.currentReward == 0)
        {
            return unitdata.upgradeCost;
        }
        return currentInfo.currentUpgradeCost;
    }
    /* 강화 배율 반환 */
    public float GetCurrentUPgradeMulti()
    {
        if (currentInfo.currentUpgradeMulti == 0)
        {
            return unitdata.upgradeMulti;
        }
        return currentInfo.currentUpgradeMulti;
...
#region 현재 스탯 기본 강화
    ...
    {
        unitdata.UpgradeHealth(amount);
    }
    /*  공격력 강화 */
    public void ADIncreasing(float amount)
    {
        unitdata.UpgradeAttackAmount(amount);
    }
    /*  공격 속도 강화 */
    public void ASIncreasing(float amount)
    {
        unitdata.UpgradeAttackSpeed(amount);
    }
    /*  방어력 강화 */
    public void ARIncreasing(float amount)
    {
        unitdata.UpgradeDefense(amount);
    }
    /*  업그레이드 비용 강화 */
    public void UpgradeCostincreasing()
    {
        unitdata.UpgradeInc();
    }
